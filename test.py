...
model_spec = importlib.util.spec_from_file_location("model", './output/' + date + '/run' + run + '/script/model.py')
model = importlib.util.module_from_spec(model_spec)
model_spec.loader.exec_module(model)

# Load the parameters of the model
params = torch.load('./output/' + date + '/run' + run + '/model/params_' + index + '.pt')
# Create a new tem model with the loaded parameters
tem = model.Model(params)
# Load the model weights after training
model_weights = torch.load('./output/' + date + '/run' + run + '/model/tem_' + index + '.pt')
...
envs = list(glob.iglob('./output/' + date + '/run' + run + '/script/envs/*'))
